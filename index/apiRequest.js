    ...
    fetch(`https://www.themealdb.com/api/json/v1/1/search.php?s=${inputValue}`)
      ...
      .then(data => {
        ...
        if (data.meals) {
          ...
          data.meals.forEach(meal => {
            ...
            function sizeChange(){
              ...
              // Check if the size is less than 767 pixels
              if(size < 767){
        ...
        // If there are no search results, display a message in the center of the container
          ...
          noResults.innerHTML = "No meals found";
...
function renderFav(){
  ...
  for(let i = 0;i < myFavoriteMeals.length;++i){
    ...
    if(myFavoriteMeals[i].loved === true){
      ...
      mealImage.addEventListener('click',() => {
        newStrMeal = myFavoriteMeals[i].src; //data.meals[0].strMeal + ' loved!';
        newMealThumb = myFavoriteMeals[i].name;//data.meals[0].strMealThumb.replace('thumb', 'preview');
        window.location.href = newStrMeal;
      });
  ...
  if(target.id === 'img'){
    ...
    getMealInfo(newStrMeal).then(mealData => {
      if (mealData) {
        ...
        mealName.innerHTML = `${mealData.mealName}`;
        ...
        //append box and instructions into the container
